...
df_sites_for_ipm_base = df_predictors %>%
  ...
  mutate(n_iterations=NUM_ITERATIONS,
         # assume a standard population at each location since we don't have data
         n_A_initial = 0.1, 
         size_mean = 20, 
         size_sd = 5) %>%
  mutate(cores=1, # do the looping over pixels not cores here, don't change this  
         progress=FALSE) %>%
  ...
  group_by(pixel) %>%
  slice(rep(1:n(), each=NUM_CLIMATE_RESAMPLES)) %>% 
...
run_scenario_spatial <- function(df, scenario_name, n_S_scale_factor=1, Tmax_scale_factor=1, SWE_scale_factor=1)
{
  ...
  climate_all_this = mclapply(1:nrow(df), function(i)
  {
  ...
  ipm_all_this = mclapply(1:nrow(df), function(i)
  {
    ...
    return(result)
  }, mc.cores=NUM_CORES) # multiprocess over pixels
...
result_scenario_climate_stress = run_scenario_spatial(df_sites_for_ipm_base,scenario_name = 'climate_stress', SWE_scale_factor = 0.8, Tmax_scale_factor = 1.1)
