...
{
  coefs = coef(model, s=model$lambda.min)
  names_this = dimnames(coefs)[[1]]
...
m_survival_all = cv.glmnet(formula=formula_survival,
...
# plot_model(m_survival_all_reduced, sort.est=TRUE) + 
#   theme_bw() + 
#   geom_hline(yintercept = 1) +
#   ggtitle('survival, standardized effect')
# plot_model(m_survival_all_reduced, type='int')
# simulateResiduals(m_survival_all_reduced) %>% plot
# dev.off()
# 
# tab_model(m_survival_all_reduced, file='output_figures/table_model_tree_level_survival.html')
# 
...
m_survival_resampled = lapply(transitions_resampled, function(df) {
  cv.glmnet(formula=formula_survival,
      data = df,
      family = 'binomial')
})

plot_coefficient_table(m_survival_resampled)
...
m_growth_all = cv.glmnet(formula=formula_growth, 
                         family='gaussian',
               data = transitions_all_filtered_joined_no_na)
...
# plot_model(m_growth_all_reduced, type='est', sort.est=TRUE, rm.terms='size') + 
#   theme_bw() + 
#   geom_hline(yintercept = 0) +
#   ggtitle('growth, standardized effect')
# plot_model(m_growth_all_reduced, type='int')
# simulateResiduals(m_growth_all_reduced) %>% plot
# dev.off()
# 
# tab_model(m_growth_all_reduced, file='output_figures/table_model_tree_level_growth.html')
# 
...
m_growth_resampled = lapply(transitions_resampled, function(df) {
  cv.glmnet(formula=formula_growth,
         family='gaussian',
      data = df)
})

plot_coefficient_table(m_growth_resampled)
...
counts_recruit = transitions_all_filtered_joined_no_na %>%
  group_by(site_code, site_type, weight, delta_years, year, n_medium_trees, Ploidy_level, geneticSexID, Cos.aspect, Elevation, STB.0, SWE.0, STB.1, SWE.1, population_density_m2) %>%
  tally(recruit) %>%
...
m_recruit_count_all = cv.glmnet(formula = formula_recruit_count,
                          data=counts_recruit,
                          family = 'poisson')
...
# plot_model(m_recruit_count_all_reduced, sort.est=TRUE) + 
#   theme_bw() + 
#   geom_hline(yintercept = 1) +
#   ggtitle('recruit count, standardized effect')
# try(plot_model(m_recruit_count_all_reduced, type='int'))
# simulateResiduals(m_recruit_count_all_reduced) %>% plot
# dev.off()
# 
# tab_model(m_recruit_count_all_reduced, file='output_figures/table_model_tree_level_recruit_count.html')
# 
...
m_recruit_count_resampled = lapply(counts_recruit_resampled, function(df) {
  cv.glmnet(formula=formula_recruit_count,
      family='poisson',
      data = df)
})

plot_coefficient_table(m_recruit_count_resampled)
...
# plot_model(m_n_medium_all_reduced, sort.est=TRUE) + 
#   theme_bw() + 
#   geom_hline(yintercept = 1) +
#   ggtitle('n medium, standardized effect')
# try(plot_model(m_n_medium_all_reduced, type='int'))
# simulateResiduals(m_n_medium_all_reduced) %>% plot
# dev.off()
# 
# tab_model(m_n_medium_all_reduced, file='output_figures/table_model_tree_level_n_medium.html')
# 
...
m_n_medium_resampled = lapply(counts_recruit_resampled, function(df) {
  # get initial theta guess
  m_n_medium_all_base = glm.nb(formula_n_medium,
                               data=df)
  
  cv.glmnet(formula = formula_n_medium,
         data=df,
         family = negative.binomial(theta=m_n_medium_all_base$theta))
})

plot_coefficient_table(m_n_medium_resampled)
...
y_pred_all = predict(m_growth_all,
                 lambda=m_growth_all$lambda.min,
...
m_growth_size_variance_resampled = lapply(1:length(m_growth_resampled), function(i) {
  y_pred = predict(m_growth_resampled[[i]],
                   lambda=m_growth_resampled[[i]]$lambda.min,
...
plot_coefficient_table(m_growth_size_variance_resampled)
...
# 
# 
# g_tree_level_rate_survival = plot(ggpredict(m_survival_all_reduced,terms=c('Cos.aspect','Elevation'))) + 
#   labs(color='Elevation') + ggtitle('Probability of survival') + ylab('') + xlab('Cos aspect')
# g_tree_level_rate_size = plot(ggpredict(m_growth_all_reduced,terms=c('Cos.aspect','Ploidy_level')),colors=c('blue','red','gray')) + 
#   labs(color='Ploidy level') + ggtitle('Size (cm)') + ylab('') + xlab('Cos aspect')
...
#   labs(color='Sex') + ggtitle('# recruits') + ylab('') + xlab('# medium trees')
# # g_tree_level_rate_prob_medium = plot(ggpredict(m_prob_medium_all_reduced,terms=c('Elevation','Ploidy_level')),colors=c('blue','red','gray')) + 
# #   labs(color='Ploidy level') + ggtitle('Prob of medium tree') + ylab('') + xlab('Elevation')
# g_tree_level_rate_n_medium = plot(ggpredict(m_n_medium_all_reduced,terms=c('Elevation','Ploidy_level'))) + 
#   labs(color='Cytotype') + ggtitle('# medium trees / population density') + ylab('') + xlab('Cosine aspect')
# 
# g_tree_level_rates = ggarrange(g_tree_level_rate_survival, g_tree_level_rate_size, g_tree_level_rate_recruit, 
#                                #g_tree_level_rate_prob_medium, 
#                                g_tree_level_rate_n_medium,
...
#                              m_prob_medium_this = m_prob_medium_all_reduced,
#                              m_n_medium_this = m_n_medium_all_reduced)
# {
...
#   if (prob_medium >= 0.5)
#   {
#     # gamma family, then need to 
#     mu = update_coefficients_full(coef(m_n_medium_this), xvar='size', other_vars = current_conditions)
#     # gamma link
#     mu_inv = 1/mu # this is the log(1+x/popdensity)
#     # invert the log(1+x/popdensity)
#     n_S = (exp(mu_inv)-1)*current_conditions$population_density_m2
#   }
#   else
#   {
#     n_S = 0
...
run_model <- function(prefix_this='test',
                      ...
                      m_n_medium_this = m_n_medium_all,
...
)
{
  ...
  n_S_initial = predict_n_medium(current_conditions, 
                                 ...
                                 m_n_medium_this = m_n_medium_this)
  n_S = n_S_initial
  ...
  for (i in 1:n_iterations)
  {
    ...
    n_S_total = predict_n_medium(current_conditions,
                                 #m_prob_medium_this = m_prob_medium_this,
                                 m_n_medium_this = m_n_medium_this) # n_medium_factor is to account for browsing, etc.
    
    # regularize
    #n_S_total = clamp(n_S_total, 0, 10)
  ...
  size_mean_final = sum(n_T_time_series[n_iterations,] * size_bins) / sum(n_T_time_series[n_iterations,])
  #sx5_final = sx(5)
  population_density_final = sum(n_T_time_series[n_iterations,])
  ...
  final_ten = df_n_density %>% filter(type=='T') %>% tail(10)
  cv_final_ten = sd(final_ten$value)/mean(final_ten$value)
  ...
  basal_area_density = sum(pi*(size_bins/2/100)^2 * n_T_final_temp)
  ...
  params_to_output = c(
                       ...
                       list(prefix_this=prefix_this,
                         ...
                         cv_final_ten=cv_final_ten)
  )
  # drop unnecessary columns for printing
  params_to_output$population_density_m2 = NULL
  params_to_output$geneticSexIDunknown = NULL
  params_to_output$Ploidy_levelunknown = NULL
  ...
  g_model = ggarrange(g1, g2, g3, g4, g_pf, g_env, nrow=3, ncol=2)
...
run_model_resampled <- function(prefix_this, cores=1, ...)
{
  mclapply(1:NUM_RESAMPLES, function(i)
  {
    ...
    run_model(
      ...
      m_n_medium_this = m_n_medium_resampled[[i]],
      #m_prob_medium_this = m_prob_medium_all_resampled[[i]],
...
# test run
model_output_test = run_model(geneticSexIDmale = '0',geneticSexIDfemale = '1',
...
sapply(model_output_test_resampled, function(x) { try(x$results$n_T_final) }) %>%
  as.data.frame() %>%
  rename(n_T_final =1) %>%
  ggplot(aes(x=n_T_final)) + 
  geom_density() +
  theme_bw()
# 
...
#              dbh=x$size_bins, 
#              density=as.numeric(tail(x$n_T,1)))
#   
#   }))
# 
...
model_output_test_drought = run_model(geneticSexIDmale = '1',
                              ...
                              n_iterations = 500,
                              SWE.sequence=seq(400,0,length.out=500+1),
                              STB.sequence=seq(3,-3,length.out=500+1))
...
params = expand.grid(geneticSexIDmale=c("0","1"), 
  ...
  mutate(geneticSexIDfemale=ifelse(geneticSexIDmale=='0','1','0')) %>%
  mutate(Ploidy_leveldiploid=ifelse(Ploidy_leveltriploid=='0','1','0'))
print(nrow(params))
...
ggplot(model_results_size_df , aes(x=dbh,y=density, color=factor(index), group=paste(replicate, index))) +
  geom_line() +
  theme_bw() +
  scale_y_sqrt() +
  facet_wrap(~index) +
...
ggplot(model_results_df, aes(x=geneticSexIDmale, color=Ploidy_leveltriploid,y=n_T_final)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(position=position_jitterdodge()) + 
  theme_bw() +
  facet_grid(Elevation~STB.mean,labeller = label_both) +
  ylab('Tagged tree density (# per m2)')


ggplot(model_results_df, aes(x=geneticSexIDmale, color=Ploidy_leveltriploid,y=basal_area_density)) +
  geom_boxplot(outlier.shape=NA) +
  geom_jitter(position=position_jitterdodge()) + 
  theme_bw() +
  facet_grid(Elevation~STB.mean,labeller = label_both) +
  ylab('Basal area density (m2 / m2)')
...
ggplot(model_results_df, aes(x=geneticSexIDmale, color=Ploidy_leveltriploid,y=n_S_final)) +
  ...
  theme_bw() +
  facet_grid(Elevation~STB.mean,labeller = label_both) +
  ylab('Sapling density (# per 3 m radius subplot)') +
  scale_y_sqrt()

ggplot(model_results_df, aes(x=n_S_final,y=n_T_final)) +
  geom_point() +
  theme_bw() +
  ylab('Tagged tree density (# per m2)') +
...
m_summary_n_T_final = lm(n_T_final ~ Ploidy_leveltriploid + geneticSexIDmale + Cos.aspect + Elevation + I(Elevation^2) + SWE.mean + STB.mean,
   data=model_results_df)

m_summary_n_S_final = lm(n_S_final ~ Ploidy_leveltriploid + geneticSexIDmale + Cos.aspect + Elevation + I(Elevation^2) + SWE.mean + STB.mean,
                         data=model_results_df)
...
df_sites_for_ipm = transitions_all_filtered_joined_no_na %>%
  ...
  slice_max(year) %>%
  unique %>%
  mutate(n_iterations=200, n_S_lag_factor=0.1, cores=5, progress=FALSE) %>%
  ...
  group_by(site_code) %>%
  mutate(prefix_this=paste(site_code, row_number(), sep='_'))

# try to map out all the sites
# 
ipms_rmbl_sites = lapply(sample(1:nrow(df_sites_for_ipm),10), function(i)
{
  cat(sprintf('%d %.3f\n',i, i/nrow(df_sites_for_ipm)))
  params_this = as.list(df_sites_for_ipm[i,])
  # make environment sequence
  # this is sqrt transformed
  params_this$SWE.sequence=rnorm(n=params_this$n_iterations+1, mean=params_this$SWE.meansqrt, sd=params_this$SWE.sdsqrt)^2
  # clamp the SWE
  params_this$SWE.sequence[params_this$SWE.sequence>500] = 500
  params_this$STB.sequence=rnorm(n=params_this$n_iterations+1, mean=params_this$STB.mean, sd=params_this$STB.sd)
  # remove unneeded columns
  params_this$SWE.meansqrt = NULL
  params_this$SWE.sdsqrt = NULL
...
ipm_rmbl_df = do.call('rbind',lapply(1:length(ipms_rmbl_sites), function(r) { do.call('rbind',lapply(ipms_rmbl_sites[[r]], function(x) { as.data.frame(x$results) %>% mutate(index=r) })) })) %>%
  mutate(site_code=sapply(strsplit(prefix_this,split='_'),head,1)) %>%
  left_join(df_site_level,by='site_code')



ggplot(ipm_rmbl_df, aes(x=Elevation,y=n_T_final,color=site_code)) +
  geom_violin()
...
df_sites_for_ipm_joined_summarized = df_sites_for_ipm_joined %>% 
  ...
  summarize(n_T.mean=mean(n_T,na.rm=TRUE), 
            n_T.sd=sd(lambda,na.rm=TRUE))
